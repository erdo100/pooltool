...
def main():
    ...
    ax[0].set_title(f"a={params.value['shot_a']}, b={params.value['shot_b']}, v={params.value['shot_v']}, theta={params.value['shot_theta']}", fontsize=14, color='black')

    # loop phi from 85 to 89 degrees
    dphi = .0
    for phi in np.linspace(phi0, phi0+dphi, 1):
        ...
        events_types = [pt.EventType.BALL_BALL]
        ...
        for event in events:
            print(event.time, event.agents[0].id, event.agents[1].id)
