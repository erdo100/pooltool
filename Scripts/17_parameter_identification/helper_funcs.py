...
def open_shotfile(file_name=None):
    ...
    for shot_actual in shots_actual:
        ball_ids, _ = get_ball_ids(shot_actual)
        ...
        # Process each ball
        for i in [1, 2]:
            ball_times = shot_actual["balls"][ball_ids[i]]["t"]
            new_times = shot_actual["balls"][ball_ids[0]]["t"][
                (shot_actual["balls"][ball_ids[0]]["t"] > ball_times[0]) & 
                (shot_actual["balls"][ball_ids[0]]["t"] < ball_times[1])
            ]

            if len(new_times) > 0:
                x0, y0 = shot_actual["balls"][ball_ids[i]]["x"][0], shot_actual["balls"][ball_ids[i]]["y"][0]
                shot_actual["balls"][ball_ids[i]]["t"] = np.concatenate((ball_times, new_times))
                shot_actual["balls"][ball_ids[i]]["x"] = np.concatenate((shot_actual["balls"][ball_ids[i]]["x"], np.full(len(new_times), x0)))
                shot_actual["balls"][ball_ids[i]]["y"] = np.concatenate((shot_actual["balls"][ball_ids[i]]["y"], np.full(len(new_times), y0)))

                sort_indices = np.argsort(shot_actual["balls"][ball_ids[i]]["t"])
                for key in ["t", "x", "y"]:
                    shot_actual["balls"][ball_ids[i]][key] = shot_actual["balls"][ball_ids[i]][key][sort_indices]

        # Create unified time array
        ballx_times = np.sort(np.unique(np.concatenate([shot_actual["balls"][ball_id]["t"] for ball_id in shot_actual["balls"]])))

        # Interpolate for all balls
        for ball in shot_actual["balls"].values():
            f_x = interp1d(ball["t"], ball["x"], kind="linear", bounds_error=False, fill_value=ball["x"][-1])
            f_y = interp1d(ball["t"], ball["y"], kind="linear", bounds_error=False, fill_value=ball["y"][-1])
            ball["x"], ball["y"], ball["t"] = f_x(ballx_times), f_y(ballx_times), ballx_times
...
def evaluate_loss(sim_env, shot_actual):
    ...
    for i, rvw in enumerate([white_rvw, yellow_rvw, red_rvw]):
        simulated_x = rvw[:, 0, 0]
        simulated_y = rvw[:, 0, 1]
        measured_t = shot_actual["balls"][i+1]["t"]
        dt = np.diff(measured_t)
        dt = np.append(dt, dt[-1])
        
        measured_x = shot_actual["balls"][i+1]["x"]
        measured_y = shot_actual["balls"][i+1]["y"]
