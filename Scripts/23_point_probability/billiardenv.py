...
class BilliardEnv:
    ...
    def prepare_new_shot(self, ball_pos, params):
        
        # ball initial position
        ball_white_xy, ball_yellow_xy, ball_red_xy = ball_pos
        ...
        #self.cue.cue_ball_id = cue_ball_id
        ...
        if True:
            phi = pt.aim.at_ball(self.system, "red", cut=params.value['shot_cut'])
        else:
        ...
        # set ball-ball collision model

        self.engine.resolver.ball_ball = pt.physics.ball_ball_models[pt.physics.BallBallModel.FRICTIONAL_MATHAVAN]()
        ...
        self.engine.resolver.stick_ball.squirt_throttle = params.value['squirt_throttle']
    ...
    def get_events(self, sim_env):

        # create dict cushion_id with 12=0, 9=1, 18=2, 3=3 to find what is assigned to 12
        cushion_id = {'12': '1', '9': '2', '18': '0', '3': '3'}
        ball_names = {"white":"W", "yellow":"Y", "red":"R"}
        events_types = [pt.EventType.BALL_LINEAR_CUSHION, pt.EventType.BALL_BALL]


        hit = {}
        for ball_id, ball_name in enumerate(ball_names):
            # Extract the events for the specified ball ID
            hit[ball_id] = {}
            hit[ball_id]['t'] = []
            hit[ball_id]['with'] = []
            events = sim_env.system.events

            # check if first event is cue hit
            event = pt.events.filter_type(events, pt.EventType.STICK_BALL)
            event = pt.events.filter_ball(event, ball_name)
            
            if len(event) > 0:            
                hit[ball_id]['t'].append(event[0].time)
                hit[ball_id]['with'].append("S")
            else:
                hit[ball_id]['t'].append(0.0)
                hit[ball_id]['with'].append("-")
                

            # Check for cushion and ball hits
            events = pt.events.filter_type(events, events_types)
            events = pt.events.filter_ball(events, ball_name)


            for event in events:
                hit[ball_id]['t'].append(event.time)
                if event.event_type == pt.EventType.BALL_BALL:
                    if event.agents[0].id != ball_name:
                        contact_ball = ball_names[event.agents[0].id]
                    elif event.agents[1].id != ball_name:
                        contact_ball = ball_names[event.agents[1].id]

                    hit[ball_id]['with'].append(contact_ball)
                else:
                    hit[ball_id]['with'].append(cushion_id[event.agents[1].id])
