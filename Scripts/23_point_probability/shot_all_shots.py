...
from parameters_shot_table import Parameters
...
def process_sample(sample, ball_pos, params):
    a, b, cut = sample
    params.value["shot_a"] = a
    params.value["shot_b"] = b
    params.value["shot_cut"] = cut


    try:
        # Fixed parameters directly in the function
        worker_env.prepare_new_shot(ball_pos, params)
        system = worker_env.simulate_shot()
        result = is_point(system)
        return result
...
def run_parallel_simulation(problem, ball_pos, params, runs, method="Sobol"):

    num_vars = len(problem["names"])

    # convert bounds to lower and upper bounds
    lower_bounds, upper_bounds = [list(col) for col in zip(*problem["bounds"])]

    if method == "Sobol":
        samples = i4_sobol_generate(num_vars, runs) # number vars, resolution
        samples = samples * (np.array(upper_bounds) - np.array(lower_bounds)) + np.array(lower_bounds)
    
    elif method == "UniformRandom":
        samples = np.random.uniform(low=lower_bounds, high=upper_bounds, size=(runs, num_vars))
    ...
    worker_func = partial(process_sample, ball_pos=ball_pos, params=params)
    
    with Pool(processes=multiprocessing.cpu_count(), initializer=init_worker) as pool:
        results = pool.map(worker_func, samples_list, chunksize=1000)
...
if __name__ == "__main__":
    ...
    method = "Sobol" # "UniformRandom", "Sobol"
    runs = 20000 
    filename = "2_18_shots"
    filename_results = filename + "_results.parquet"
    filename_probability = filename + "_probability.parquet"

    white_pos = (0.5/8*2.82, 4/8*2.84)
    yellow_pos = (1/8*2.84, 7/8*2.84)
    red_pos = (1/8*2.84, 0.04)
    ball_pos = (white_pos, yellow_pos, red_pos)
    ...
    if runsims == True:
        ...
        shots_df = run_parallel_simulation(problem, ball_pos, params, runs, method)
    
        #shots_df.to_parquet(filename_results)

        # print runtime
        print("Runtime of run_parallel_simulation is", time.time() - start)
    ...
    if calculate_density == True:
        ...
        grid_axes, total_probability = calculate_probability_map(
            ...
            result_col="point",
            stddev_dict={"side": 0.04, "vert": 0.04, "cut": 7},
            steps=100,
