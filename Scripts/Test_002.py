...
results = np.zeros(len(vel))
avels0 = np.zeros(len(vel))
avels1 = np.zeros(len(vel))
avels2 = np.zeros(len(vel))
avels0r = np.zeros(len(vel))
avels1r = np.zeros(len(vel))
avels2r = np.zeros(len(vel))
avel_v = np.zeros(len(vel))
...
for vel_i in vel:
    ...
    system.cue.set_state(V0=vel_i, phi=direction, a=side, b=top)
    ...
    avel_v[i] = rvw_cue[1, 1, 1] / (0.0615 * 3.141) * 2 * 3.141
...
# Plot the results over input using matplotlib
fig2, ax2 = plt.subplots()
ax2.plot(vel, results, marker="o", linestyle="-", color="b")
...
ax3.plot(vel, avels0, marker="d", linestyle="none", color="k", label="avel[0]")
ax3.plot(vel, avels1, marker="o", linestyle="none", color="b", label="avel[1]")
ax3.plot(vel, avels2, marker="o", linestyle="none", color="r", label="avel[2]")
ax3.grid(True)
ax3.legend()
ax3.set_xlabel("Shot speed in m/s")
ax3.set_title("Angular Velocity at cushion hit")

# Plot the results over input using matplotlib
fig4, ax4 = plt.subplots()
ax4.plot(
    vel, avels0r, marker="d", linestyle="none", color="k", label="rvw_cue[1, 2, 0]"
)
ax4.plot(
    vel, avels1r, marker="o", linestyle="none", color="b", label="rvw_cue[1, 2, 1]"
)
ax4.plot(
    vel, avels2r, marker="o", linestyle="none", color="r", label="rvw_cue[1, 2, 2]"
)
ax4.set_xlabel("Shot speed in m/s")
ax4.grid(True)
ax4.legend()
...
# ax5.plot(vel, avels0r/avel_v, marker='d', linestyle='none', color='k', label="rvw_cue[1, 2, 0]/v*R")
# ax5.plot(vel, avels1r/avel_v, marker='o', linestyle='none', color='b', label="rvw_cue[1, 2, 1]/v*R")
ax5.plot(
    vel,
    avels2r / avel_v,
    marker="o",
    linestyle="none",
    color="r",
    label="rvw_cue[1, 2, 2]/v*R",
)
ax5.set_xlabel("Shot speed in m/s")
ax5.set_ylim([-0.7, -0.5])

ax5.grid(True)
ax5.legend()
ax5.set_title('Spin Rate Factor at start for side "a" = 0.5')
