...
from optim_tools import run_study, run_simulation
...
class plot_3cushion():
    def __init__(self, params):
        ...
        # Add a button to save the system
        run_study_button = Button(slider_frame, text="Run", command=lambda: run_study(self.shots_actual, self.params))
        ...
        for i in range(3):
            ...
            handles["varline_actual"][i] = ax[i+1].plot(actual_times, actual_v, label=f"{colortable[i]} actual v in m/s", linestyle='--', linewidth=1)
            handles["varline_simulated"][i] = ax[i+1].plot(tsim, simulated_v, label=f"{colortable[i]} simulated v in m/s", linestyle='-', linewidth=1)#, marker='o')
    ...
    def update_plot(self, event=None):
        ...
        params.value['ballball_a'] = sliders['ballball_a'].get()
        params.value['ballball_b'] = sliders['ballball_b'].get()
        params.value['ballball_c'] = sliders['ballball_c'].get()
        ...
        system, tsim, white_rvw, yellow_rvw, red_rvw = run_simulation(ball_cols, ball_xy_ini, params)
